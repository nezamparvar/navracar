    ...
    <form method="POST" action="{{ route('admin.invoices.store') }}"
        ...
        <x-card title="اطلاعات مشتری و سند" icon="invoice">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div><label class="mb-1 block text-xs font-bold text-ink-500">شماره شاسی</label><input name="vin" value="{{ old('vin', $prefill['vin']) }}" class="w-full rounded-xl border border-ink-200 bg-ink-50 px-3.5 py-2.5 text-sm dark:border-white/10 dark:bg-white/5"></div>
            </div>
        </x-card>
        ...
        <x-card title="تخفیف و نهایی‌سازی" icon="check">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div><label class="mb-1 block text-xs font-bold text-ink-500">تخفیف (تومان)</label><input type="number" min="0" step="1" name="discount_amount" value="{{ old('discount_amount', 0) }}" class="w-full rounded-xl border border-ink-200 bg-ink-50 px-3.5 py-2.5 num-font dark:border-white/10 dark:bg-white/5"></div>
                <div><label class="mb-1 block text-xs font-bold text-ink-500">جمع کل محاسبه‌شده توسط سرور/ردیف‌ها</label><div class="rounded-xl border border-ink-200 bg-ink-50 px-3.5 py-2.5 num-font font-extrabold dark:border-white/10 dark:bg-white/5" x-text="fmt(displayTotal) + ' تومان'"></div></div>
            </div>
            <div class="mt-4"><label class="mb-1 block text-xs font-bold text-ink-500">شرایط پرداخت</label><textarea name="payment_terms" rows="3" class="w-full rounded-xl border border-ink-200 bg-ink-50 px-3.5 py-2.5 text-sm dark:border-white/10 dark:bg-white/5">{{ old('payment_terms', $prefill['payment_terms'] ?? '') }}</textarea></div>
