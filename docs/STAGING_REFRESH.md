...
3. If realistic images/PDFs are needed, copy a reviewed production upload snapshot into `/home/navrac/navracar-staging-app/storage/app/public` using File Manager. Do not copy into production and do not replace the `/home/navrac/public_html/staging/storage` link/path.
4. Restore staging-only settings, test users, and integration flags. Do not restore Directory Privacy or HTTP Basic Auth; the staging `.env` is never overwritten by a data refresh.
