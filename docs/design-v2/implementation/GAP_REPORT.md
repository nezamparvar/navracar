...
- **Content dashboard:** `ContentDashboardController` extended with a review queue (`ImportQueueItem` where `status = 'needs_review'`, decorated with title/source/image-count from `parsed_json` + `images_imported`), a deterministic per-item quality score (present-criteria-count ÷ 8 criteria × 100 — the 7 `QUALITY_FIELDS` text/numeric fields plus "has at least one imported image" as the 8th criterion, no randomness — corrected in round-4-remediation-r2 after the owner caught the doc comment claiming an image criterion that the formula didn't actually include), a content-health widget (per-field completion % across the real `CarListing` catalog — title/brand+model/engine/price/meta-title/meta-description/images — averaged for the overall score, 0% on an empty catalog rather than a misleading 100%), urgent tasks (incomplete meta, failed imports, listings without images, drafts), a real weekly publish-activity chart (`CarListing`/`Post.published_at`, `HomeSlide.created_at`, grouped by actual date), and a content summary (real counts).
