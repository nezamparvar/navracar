...
**Partially implemented (round 4, commit `71362bd`):** a new `content.role`-gated `ContentDashboardController`/`admin.content-dashboard` route/view now exists, with real KPIs from existing data (published/draft `CarListing` counts, `ImportQueueItem` counts needing review or failed, published/draft `Post` counts, active `HomeSlide` count) and recent-listings/recent-posts lists — content managers now land here after login instead of on a bare table with no overview at all. Sales users now land on `admin.dashboard` after login instead of skipping it.
**Correction after actually opening `03-sales-dashboard.png` and `04-content-dashboard.png` (round 4, later in the same session):** an earlier pass of this document marked both dashboards "RESOLVED" — that overstated it. Both reference images show a **role-specific reduced sidebar** (sales: داشبورد فروش/پایپلاین/درخواست‌ها/پیش‌فاکتورها/تقویم/پیگیری‌های من/قالب‌های پیام only — no content/admin-only groups; content: داشبورد محتوا/آگهی‌ها و ایمپورت/صف بررسی/وبلاگ/اسلایدر/منو/رسانه‌ها/قالب متا only), not the shared full admin sidebar both roles currently see. They also show widgets with no equivalent built yet: sales — "برنامه امروز" (today's schedule), "پیگیری‌های عقب‌افتاده" (overdue follow-ups), "نرخ تبدیل این ماه" (a سرنخ→مشاوره→پیش‌فاکتور→فروش conversion funnel); content — "صف بررسی آگهی‌ها" (a review queue table with a per-listing data-quality score bar), "سلامت محتوای آگهی‌ها" (a content-health radial + per-field completeness bars), "انتشار محتوا" (a publish-activity chart), "کارهای فوری" (urgent tasks).
**Why not built now:** the sidebar variant touches the shared `layouts/admin.blade.php` used by every admin page — building it carelessly risks breaking navigation everywhere else, not just these two dashboards. The funnel and content-health-score widgets don't have a clean, non-fabricated data mapping to the current schema (e.g. no "مشاوره" stage exists between "سرنخ" and "پیش‌فاکتور" in the real `PipelineStage` set; no per-field-completeness scoring exists on `CarListing`) — inventing plausible-looking numbers for either would be exactly the kind of fabrication this project's rules forbid. "برنامه امروز" and "پیگیری‌های عقب‌افتاده" *are* honestly buildable from existing data (`CalendarEvent` for today, `QuoteRequest.next_call_date` for overdue) and are the concrete next step.
**Evidence:** `docs/design-v2/assets/03-sales-dashboard.png`, `docs/design-v2/assets/04-content-dashboard.png`, `app/Http/Controllers/Admin/ContentDashboardController.php`, `resources/views/components/layouts/admin.blade.php`.
**Follow-up:** (1) build "برنامه امروز" and "پیگیری‌های عقب‌افتاده" as real widgets on the sales-visible dashboard — no data gap, just not yet built; (2) a role-scoped reduced sidebar variant, done carefully with regression coverage on existing admin nav tests; (3) the conversion-funnel and content-health-score widgets need an explicit product decision on what data backs them (a real "مشاوره" pipeline stage? a real per-field completeness rule for listings?) before they can be built honestly — flagged for the product owner, not guessed at here.
