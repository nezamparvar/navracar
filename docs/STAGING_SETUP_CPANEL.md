...
4. In File Manager, create `/home/navrac/navracar-staging-app` and its `storage/` tree. Create a separate `.env` there from `.env.example` with staging-only values.
   The deployment script prepares these persistent runtime directories before activating each release: `storage/app/public`, `storage/framework/cache/data`, `storage/framework/sessions`, `storage/framework/views`, and `storage/logs`. They may be absent from the Git artifact because Git does not track empty directories. Existing files, uploads, logs, sessions, and caches are never copied, deleted, or replaced. The cPanel/PHP account must be able to write each path or deployment stops before the release swap.
