<x-card title="عنوان و شناسه" icon="car" variant="v2" x-data="carListingForm">
    <div class="space-y-3">
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">عنوان فارسی</label>
        ...
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">اسلاگ (آدرس صفحه)</label>
            ...
            @if($l->exists)
                <p class="mt-1 text-[11px] text-v2-text-muted">{{ url('/car-prices/'.$l->slug) }}</p>
            @else
                <p class="mt-1 text-[11px] text-v2-text-muted">پیش‌نمایش: <span x-text="previewSlug || 'خودکار ساخته می‌شود'"></span></p>
            @endif
        </div>
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div>
                <label class="mb-1 block text-xs font-bold text-v2-text-muted">مارک</label>
                <input type="text" name="make" value="{{ old('make', $l->make) }}" list="makes-list" dir="ltr" autocomplete="off"
                       class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3 py-2 text-sm text-left text-v2-text">
            ...
            <div>
                <label class="mb-1 block text-xs font-bold text-v2-text-muted">مدل</label>
                <input type="text" name="model" value="{{ old('model', $l->model) }}" dir="ltr" autocomplete="off"
                       class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3 py-2 text-sm text-left text-v2-text">
            </div>
            <div>
                <label class="mb-1 block text-xs font-bold text-v2-text-muted">تیریم</label>
                <input type="text" name="trim_level" value="{{ old('trim_level', $l->trim_level) }}" dir="ltr"
                       class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3 py-2 text-sm text-left text-v2-text">
            </div>
            <div>
                <label class="mb-1 block text-xs font-bold text-v2-text-muted">سال ساخت</label>
                <input type="text" name="model_year" value="{{ old('model_year', $l->model_year) }}" dir="ltr"
                       class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3 py-2 text-sm text-v2-text">
            </div>
        </div>
    </div>
</x-card>
...
<x-card title="قیمت و دسته‌بندی خودرو" icon="target" variant="v2"
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">قیمت (درهم امارات)</label>
            <input type="number" step="1" name="price_aed" x-model.number="realPrice" value="{{ old('price_aed', (float) $l->price_aed) }}" required
                   class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3.5 py-2.5 text-sm num-font text-v2-text">
        </div>
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">قیمت گمرکی خودرو (درهم)</label>
            ...
            <div class="mt-2 flex items-center justify-between">
                <p class="text-[11px] text-v2-text-muted">
                ...
                <button type="button" x-show="customsPriceTouched && suggestedCustomsPrice > 0" @click="restoreSuggestion"
                        class="text-xs font-bold text-v2-primary hover:underline">استفاده از پیشنهاد</button>
            </div>
        </div>
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">دسته‌بندی خودرو</label>
            <select name="category_id" class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3.5 py-2.5 text-sm text-v2-text">
            ...
            <p class="mt-1 text-[11px] text-v2-text-muted">درصدها از <a href="{{ route('admin.settings.edit') }}" class="text-v2-primary hover:underline">تنظیمات نرخ‌ها</a> خوانده می‌شوند.</p>
        </div>
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">مدت زمان تحویل (روز کاری)</label>
...
<x-card title="مشخصات فنی" icon="list" variant="v2">
    @php
        $specFields = [
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        @foreach ($specFields as $field => $label)
            <div>
                <label class="mb-1 block text-xs font-bold text-v2-text-muted">{{ $label }}</label>
...
<x-card title="سئو" icon="globe" variant="v2">
    <div class="space-y-3">
        <div>
            ...
            <input type="text" name="meta_title" value="{{ old('meta_title', $l->meta_title) }}" class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3.5 py-2.5 text-sm text-v2-text">
        </div>
        <div>
            <label class="mb-1 block text-xs font-bold text-v2-text-muted">متا-دیسکریپشن</label>
            <textarea name="meta_description" rows="2" class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3.5 py-2.5 text-sm text-v2-text">{{ old('meta_description', $l->meta_description) }}</textarea>
        </div>
    </div>
</x-card>
