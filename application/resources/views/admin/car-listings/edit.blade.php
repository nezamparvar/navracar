...
<x-layouts.admin :page-title="$pageTitle" :page-subtitle="$l->source_url ? 'منبع: '.$l->source_url : 'آگهی دستی (بدون منبع دابیزل)'">
    ...
    <div class="mb-5 flex flex-wrap items-center gap-2">
        <x-badge :color="$l->status === 'published' ? 'v2-success' : 'v2-neutral'">
            {{ $l->status === 'published' ? 'منتشرشده' : 'پیش‌نویس' }}
        </x-badge>

        @if ($l->status === 'published')
            <x-button :href="route('public.car-prices.show', $l)" target="_blank" size="sm" variant="v2-secondary">
                <x-icon name="external-link" class="w-4 h-4" /> مشاهده در سایت
            </x-button>
            <form method="POST" action="{{ route('admin.car-listings.unpublish', $l) }}">
                @csrf
                <x-button type="submit" size="sm" variant="v2-secondary">
        ...
        @else
            ...
            <form method="POST" action="{{ route('admin.car-listings.publish', $l) }}">
                @csrf
                <x-button type="submit" size="sm" variant="v2-primary">
        ...
        <form method="POST" action="{{ route('admin.car-listings.destroy', $l) }}"
            ...
            @csrf @method('DELETE')
            <x-button type="submit" size="sm" variant="v2-danger">
    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-5">
            <form method="POST" action="{{ route('admin.car-listings.update', $l) }}" class="space-y-5">
                ...
                <x-card title="عکس‌ها" icon="image" variant="v2">
                    @if ($l->images->isEmpty())
                        <x-empty-state icon="image" title="عکسی دریافت نشد — از فرم زیر عکس اضافه کنید." variant="v2" />
                    @else
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                            @foreach ($l->images as $img)
                                <div class="overflow-hidden rounded-xl border border-v2-border">
                                    <img src="{{ $img->url() }}" alt="" class="h-28 w-full object-cover">
                                    <div class="space-y-1.5 p-2">
                                        <label class="flex items-center gap-1.5 text-[11px] font-bold text-v2-text-muted">
                                        ...
                                        <input type="number" name="sort_order[{{ $img->id }}]" value="{{ $img->sort_order }}"
                                               class="w-full rounded-lg border border-v2-border bg-v2-elevated px-2 py-1 text-[11px] text-v2-text">
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <x-card title="افزودن عکس دستی" icon="upload" variant="v2">
                <form method="POST" action="{{ route('admin.car-listings.images.store', $l) }}" enctype="multipart/form-data" class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="min-w-[220px] flex-1">
                        <label class="mb-1 block text-xs font-bold text-v2-text-muted">لینک مستقیم عکس</label>
                        <input type="url" name="image_url" placeholder="https://..." dir="ltr" class="w-full rounded-xl border border-v2-border bg-v2-elevated px-3 py-2 text-sm text-left text-v2-text">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-bold text-v2-text-muted">یا آپلود فایل</label>
        ...
        <div class="space-y-5">
            @if ($l->images->isNotEmpty())
                <x-card title="حذف عکس" icon="trash" variant="v2">
                    <div class="space-y-2">
                        @foreach ($l->images as $img)
            ...
            <x-card title="توضیحات اصلی (انگلیسی، فقط مرجع)" icon="info" variant="v2">
                <p class="max-h-72 overflow-y-auto whitespace-pre-wrap text-xs leading-6 text-v2-text-muted" dir="ltr">
                    {{ $l->description_en }}
                </p>
            </x-card>
        </div>
