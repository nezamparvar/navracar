<x-layouts.admin :page-title="$pageTitle">
    ...
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            <x-button :href="route('admin.requests.create')" variant="amber" size="sm" class="min-h-[44px]"><x-icon name="plus" class="w-4 h-4" /> ثبت دستی مشتری تماس‌گرفته</x-button>
            <x-button :href="route('public.lead-form')" variant="secondary" size="sm" target="_blank" class="min-h-[44px]"><x-icon name="external-link" class="w-4 h-4" /> لینک فرم عمومی فروشنده‌ها</x-button>
        </div>
    </div>
        ...
        <form method="GET" class="mb-5 space-y-3">
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="name" value="{{ $filters['name'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-ink-500 dark:text-ink-400">تلفن</label>
                    <input type="text" name="phone" value="{{ $filters['phone'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-ink-500 dark:text-ink-400">ایمیل</label>
                    <input type="text" name="email" value="{{ $filters['email'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-ink-500 dark:text-ink-400">خودرو</label>
                    <input type="text" name="car_label" value="{{ $filters['car_label'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="date" name="from" value="{{ $filters['from'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-ink-500 dark:text-ink-400">تا تاریخ</label>
                    <input type="date" name="to" value="{{ $filters['to'] ?? '' }}" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-ink-500 dark:text-ink-400">وضعیت پیگیری</label>
                    <select name="status" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select name="stage" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
                        <option value="">همه</option>
            ...
            @if (auth()->user()->isAdmin())
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-2">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="assigned" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/5">
            ...
            <div class="flex flex-wrap gap-2">
                <x-button type="submit" size="sm" class="min-h-[44px]">اعمال فیلتر</x-button>
                <x-button :href="route('admin.requests.index')" variant="secondary" size="sm" class="min-h-[44px]">پاک کردن</x-button>
                <x-button :href="route('admin.export', array_merge(['type' => 'requests'], $filters))" variant="amber" size="sm" class="min-h-[44px]">خروجی اکسل</x-button>
            </div>
        </form>
                    ...
                    <tbody>
                        @foreach ($rows as $r)
                            <tr class="border-b border-ink-100 hover:bg-ink-50 dark:border-white/5 dark:hover:bg-white/5">
                                <td class="whitespace-nowrap px-2.5 py-2.5 text-xs text-ink-500" data-label="تاریخ">{{ $r->created_at->format('Y-m-d H:i') }}</td>
                                ...
                                <td class="px-2.5 py-2.5" data-label="منبع"><x-badge>{{ $r->source ?: 'سایت' }}</x-badge></td>
                                ...
                                <td class="px-2.5 py-2.5 flex flex-wrap gap-1" data-label="عملیات">
                                    ...
                                    @if (!$r->is_archived)
                                        <form method="POST" action="{{ route('admin.requests.archive', $r) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="min-h-[44px] rounded-lg border border-ink-200 bg-ink-50 px-2 py-1 text-xs font-semibold text-ink-700 hover:bg-ink-100 dark:border-white/10 dark:bg-white/5 dark:text-ink-200 dark:hover:bg-white/10">بایگانی</button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('admin.requests.unarchive', $r) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="min-h-[44px] rounded-lg border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700 hover:bg-amber-100 dark:border-amber-900/30 dark:bg-amber-900/10 dark:text-amber-200">خارج</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
